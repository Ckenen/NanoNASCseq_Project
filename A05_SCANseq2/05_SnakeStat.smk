...
OUTDIR = "results/05_stat"

rule all:
    input:
        expand(OUTDIR + "/01_read_location/{run_cell}_summary.tsv", run_cell=RUN_CELLS),
...
rule stat_read_location:
    ...
    output:
        bed1 = temp(OUTDIR + "/01_read_location/{run}/{cell}.bed"),
        bed2 = OUTDIR + "/01_read_location/{run}/{cell}.bed.gz",
        tsv = OUTDIR + "/01_read_location/{run}/{cell}_summary.tsv",
    log:
        OUTDIR + "/01_read_location/{run}/{cell}.log"
    shell:
        """
